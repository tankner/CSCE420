...
class PerceptronModel(object):
    ...
    def train(self, dataset):
        ...
        while not all_correct:
            all_correct = True
            for features, correct_labels in dataset.iterate_once(1):
                if self.get_prediction(features) != nn.as_scalar(correct_labels):
                    all_correct = False
                    self.w.update(features, nn.as_scalar(correct_labels))
